...
* A running Hub instance — a local test instance from the [Quick Start](../self-hosting-guide/quick-start.mdx) or a server deployment (managed or selfhosted)
...
Bob and Carol can now log in and complete their account setup, as described in the [User Guide](../user-guide/quick-start.mdx#set-up-your-account).
...
For more details, read [Emergency Access admin settings](emergency-access.mdx#admin-settings), [Set Up Emergency Access](emergency-access.mdx#set-up-emergency-access), and the per-vault [Emergency Access Council](../user-guide/vault-management.mdx#emergency-access-council).
...
For more details, read [Audit Logs](audit-logs.mdx), [Filtering Audit Logs](audit-logs.mdx#filtering-audit-logs), and the list of [Event Types](audit-logs.mdx#event-types).
...
For more details, read [License](license.mdx), [What Is a Seat?](license.mdx#what-is-a-seat), and [Updating Your License](license.mdx#updating-your-license).

## Next Steps {/* #next-steps */}

* Set up [backups](../self-hosting-guide/operations.mdx#backup) before real data accumulates.
...
* Send your team the [User Guide](../user-guide/quick-start.mdx) so they can get started on their own.
